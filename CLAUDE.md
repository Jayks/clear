...
- **`GlobalFab`** — fan-out FAB, Home page only (`!isEmpty`), `bottom-nav-safe right-4 z-50`; Log expense → `GroupPickerSheet` → **mode chooser (`LogExpenseTiles`: Scan/Voice/Type)** → `QuickAddSheet` with the chosen `startMode`; Log entry → `StreamLogSheet`. See `components/CLAUDE.md`.
