...
- **`GlobalFab`** — destination-first FAB, Home page only (`!isEmpty`), `bottom-nav-safe right-4 z-50`, cyan gradient. Tapping it opens the unified **"Add to…"** `GroupPickerSheet` directly (no expense/entry fan — "entry" was confusing jargon). Pick a **group** → mode chooser (`LogExpenseTiles`: Scan/Voice/Type) → `QuickAddSheet`. Pick a **person** (People section, shown only when `hasStreams`) → `StreamLogSheet` with `preselectedPerson`. Section layout decided by pure `resolvePickerSections()` (`global-fab-logic.ts`, unit-tested). See `components/CLAUDE.md`.
- **`LogExpenseTiles`** (`components/expense/log-expense-tiles.tsx`) — shared Scan/Voice/Type chooser + `StartMode` type; single source of truth used by `GroupActionHub` Zone 1 **and** the `GlobalFab` mode-chooser step so the "how to log" surface is identical across entry points (`ExpenseQuickAddFab`'s fan uses the same palette/icons).
- **`GroupActionHub`** (`components/trip/group-action-hub.tsx`) — 3-zone action hub replacing `TripCardNavSheet` + `TripCardQuickAdd`. Zone 1 (Log expense): `LogExpenseTiles` (Scan/Voice/Type) that open `QuickAddSheet` with `startMode` prop (`"scan"` auto-opens scanner, `"voice"` auto-starts mic after 350ms, `"text"` normal). Zone 2 (Jump to): Trips/Nests = 4 tiles; Circles = Expenses+Members only (no Settle/Insights). Zone 3 (Manage): Edit · Archive/Unarchive (admin-only) + Share (all members — visible whenever `joinUrl` is present). Zone 3 is hidden entirely when the user is not admin and `joinUrl` is absent. Entry points: `⋯` button on cards, 500ms long-press on cards, `GroupMobileNav`, `GroupHeroHub`. Circle-aware: Zone 1 hidden entirely for `groupType === "circle"`.
