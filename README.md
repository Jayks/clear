...
ClearOff is a shared expense and personal debt tracking app. Log what each person paid, choose how to split it, and let Clear compute who owes whom — with the minimum number of payments. No group needed for everyday debts.
...
Three-destination structure: **Home** (Trips + Nests + Circles) · **Streams** · **Insights**. On mobile this is a bottom tab bar; on desktop it's a collapsible left sidebar with **Trips** · **Nests** · **Circles** as first-class section shortcuts at the top (click once to jump straight to that group type, no home-screen hop) and **Streams** · **Insights** below — both surfaces carry the same unread/dispute indicator on Streams.

The Home page has an **Active / Archived** underline-tab toggle above the section pills. Active view shows Trips · Nests · Circles with colour-coded headers and a sticky pill nav; each section can be collapsed individually so you see only what matters right now. Archived view shows the same type groupings at full opacity. The tab row doubles as a search bar — a 🔍 icon expands to a full search input; blurring with a query collapses it to a filter chip `[🔍 query ×]` so the tabs stay accessible.
...
- **Global quick-add FAB** — cyan gradient `+` button on the Home page (shown when groups exist). Tapping it opens a **destination-first** group picker ("Add to…") — no expense/entry fan. Pick a **group** → Scan / Voice / Type mode chooser → quick-add sheet. Pick a **person** (Streams users only) → stream entry sheet. Auto-hides scrolling down, reappears on scroll up.
...
- **Undo settlement** — "Mark paid" shows a 5-second toast with an Undo action; tapping it deletes the settlement and refreshes the page, preventing accidental payments from becoming permanent. A **settlement confidence callout** on the settle page confirms who paid whom and the amount immediately after marking — no ambiguity about whether it went through
...
- **Trip Memories** — two-tier photo feature for trips. **Free tier**: paste a Google Photos / iCloud / any album link from the trip Edit page — a link card (rose icon, domain label) appears on both the group overview and the public summary page. **Plus tier**: upload photos directly into ClearOff — gallery-density grid (3/4/5 columns), tap to open fullscreen lightbox (swipe · keyboard arrows · backdrop-tap to close), caption + uploader attribution in the lightbox footer, 30-photo cap. Both tiers show together on the public trip summary page under one "Memories" section.
...
- **Group creation** — type selector shows clear descriptions ("One-time trips & events" / "Home, flatmates & recurring"); optional fields (cover photo, dates) labelled upfront; budget, description, and itinerary hidden behind `+ More options` for a clean first-time flow. Brand-new groups open with a short **first-run checklist** — add members, log the first expense — so the blank-slate start is guided
...
13. Run `drizzle/razorpay-tables.sql` then `drizzle/razorpay-mode-column.sql` to add ClearOff Plus / Razorpay payment support (`subscriptions` columns, `razorpay_payments`, `razorpay_webhook_events`)
...
18. Run `drizzle/trip-photos.sql` to create the `trip_photos` table (Trip Memories upload feature). Apply via the Supabase SQL Editor or a Node postgres client — **not** `pnpm db:push` (drizzle-kit push fails on this project due to a CHECK constraint bug on `group_members`).
19. Create a Storage bucket named `trip-photos` (public, 10 MB limit). No per-user storage RLS INSERT policy needed — `app/actions/trip-photos.ts` uses the service-role admin client (`createAdminClient()`) for all upload URL generation and deletes, so bucket-level public read access is sufficient.
20. Generate VAPID keys: `node -e "const wp=require('web-push');console.log(wp.generateVAPIDKeys())"` and add to `.env.local`
21. (Optional, for real payments) Create a [Razorpay](https://razorpay.com) account, generate a test-mode key pair, and add `RAZORPAY_MODE=test` + `RAZORPAY_TEST_KEY_ID`/`RAZORPAY_TEST_KEY_SECRET` to `.env.local` — the app fails open without these (checkout shows "Payments are not configured yet" rather than crashing)
...
pnpm seed             # seed Pan-India Explorer trip demo data
