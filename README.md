...
- **Group card identity patterns** — Trip and Nest cards without a cover photo use a pale themed gradient + SVG silhouette pattern instead of a flat colour: **Trip** = soft green (`emerald-50 → green-100`) with pine tree silhouettes in emerald; **Nest** = soft blue (`sky-50 → blue-100`) with apartment building silhouettes (window grids) in sky; **Circle** = pale indigo (recurring) or amber (one-time) with sine-wave or lollipop patterns. Patterns are `repeat-x` anchored at the bottom so the silhouette rises from the card base. Two overlay divs (`dark:hidden` / `hidden dark:block`) handle light-mode coloured shapes and dark-mode white shapes separately, matching each group type's visual identity.
