...
**Two breakpoint-swapped top-nav variants, never both visible at once (June 2026):** below `sm:` a bespoke `<nav className="sm:hidden …">` — `isHome` (`usePathname() === "/"`) picks a plain icon-only logo (mobile's own `/` *is* this carousel, so a link back to itself is a dead no-op) vs. a chevron-back logo `Link` to `/` (when reached via `/about`); right side has `ThemeToggle`, a **`Home` escape hatch** (→ `/?view=full`, the only route a phone has to the full landing page — there is no other link to it from a mobile UA, since `/about` always shows this carousel regardless of device), `Sign in`, `Get started`. At `sm:`+ a plain `<div className="hidden sm:block">` renders **`<MarketingNav current={isHome ? "home" : "about"} />`** — the same shared nav every other marketing page uses, so logo placement and the link cluster are pixel-identical across `/`, `/about`, `/pricing`, `/changelog` once you're not on a phone. The bottom bar used to also carry Home/Pricing ghost-chips — **removed**, both because they were redundant once nav moved to the top and because a fixed-bottom zone on this component's `inset-0` fullscreen layout is exactly where a mobile browser's own UI chrome (Safari's bottom toolbar, etc.) can overlap and silently eat taps; the top nav has no such risk.
