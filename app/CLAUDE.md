...
**Known dead code (found, not fixed, during the M4 build)**: `split-editor.tsx`'s `UpgradePrompt` lock state can never trigger (`canUseNonEqualSplit` is hardcoded `true` since the June 2026 free re-cut) and `chat-import-dialog.tsx`'s almost never does (its `canUseAI` prop is actually fed `canUseLoggingAI`, free for everyone). Candidates for a future cleanup pass — `template-section.tsx`'s `UpgradePrompt` (gated on the still-real `canUseTemplates`) is the only live one of the three.
