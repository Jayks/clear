...
const steps = [
  { n: "01", icon: LayoutGrid, title: "Create a group",   body: "Choose Trip for travel or Nest for home expenses. Add members in seconds." },
  { n: "02", icon: Users,      title: "Invite your people", body: "Share a link or QR code. Everyone joins instantly — no account needed. Add guests by name; they claim with Google later. Or import the same crew from a past trip — zero re-typing." },
  { n: "03", icon: Receipt,    title: "Log expenses",      body: "Tap + on any card and type what you spent — AI parses the amount, payer, and split automatically. Or paste a group chat and import all expenses at once." },
  { n: "04", icon: Zap,        title: "Settle up",         body: "See exactly who owes what. Mark payments done with one tap." },
];
    ...
    <div className="overflow-x-clip">
      ...
      <nav className="glass-nav sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
          ...
          <div className="flex gap-8 shrink-0 animate-marquee">
            {tickerItems.concat(tickerItems).map((label, i) => (
              <span key={i} className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400">
                <span className="w-1 h-1 rounded-full bg-cyan-400 shrink-0" /> {label}
              </span>
            ))}
          </div>
        </div>
      ...
      <section id="how-it-works" className="max-w-6xl mx-auto px-6 py-24">
        <FadeIn className="text-center mb-14">
          ...
          <h2 className="text-4xl sm:text-5xl text-slate-800 dark:text-slate-100 mb-4" style={{ fontFamily: "var(--font-fraunces)" }}>Up and running in minutes</h2>
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-lg mx-auto">No setup, no onboarding form. Create a group and go.</p>
        </FadeIn>
          ...
          {steps.map((step, i) => (
            <FadeIn key={step.n} delay={i * 90}>
              <div className="glass rounded-2xl p-6 flex flex-col gap-4 relative">
                <div className="w-12 h-12 rounded-full flex items-center justify-center font-bold text-white text-sm shadow-lg shrink-0 relative z-10" style={{ background: "linear-gradient(135deg, #0891B2, #14B8A6)" }}>{step.n}</div>
                <div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <FadeIn className="text-center mb-14">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-xl mx-auto">Choose what fits your situation. Both use the same splitting engine and settlement optimizer.</p>
        </FadeIn>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FadeIn direction="left" className="glass rounded-3xl overflow-hidden">
            ...
            <div className="p-6">
              <ul className="space-y-3">
                {["Hotels, meals, transport — all in one place", "Trip dates, cover photo, shared itinerary", "Visual timeline of expenses and events", "AI travel narrative and budget tracking", "Daily spend chart and trip insights"].map((item) => (
          ...
          <FadeIn direction="right" className="glass rounded-3xl overflow-hidden">
            ...
            <div className="p-6">
              <ul className="space-y-3">
                {["Rent, utilities, subscriptions — split monthly", "Recurring templates: log with one tap", "Expenses grouped by month for clarity", "Monthly context on every settle-up"].map((item) => (
                  <li key={item} className="flex items-start gap-2.5 text-sm text-slate-600 dark:text-slate-300">
                    <CheckCircle2 className="w-4 h-4 text-teal-500 shrink-0 mt-0.5" /> {item}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section id="why-clear" className="max-w-6xl mx-auto px-6 pb-24">
        ...
        <FadeIn>
          <div className="glass rounded-2xl overflow-hidden mb-10">
            <div className="grid grid-cols-2 border-b border-slate-100 dark:border-slate-700/60">
              <div className="px-6 py-4 border-r border-slate-100 dark:border-slate-700/60"><p className="text-sm font-semibold text-slate-400 dark:text-slate-500">Other apps</p></div>
              <div className="px-6 py-4 bg-cyan-50/50 dark:bg-cyan-950/20"><p className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">Clear ✦</p></div>
            </div>
            {[
              { them: "Type every field manually",                         us: "AI parses amount, payer and split in seconds" },
              { them: "Everyone must create an account to join",           us: "Add guests by name — they claim with Google later" },
              { them: "Complex chains of IOUs between everyone",           us: "Minimum transactions — one payment per person, guaranteed" },
              { them: "No visual way to see who owes whom",                us: "Debt Flow graph — animated money flows, tap any arc to pay instantly" },
              { them: "Expenses pile up silently",                         us: "Email + push the moment any money moves" },
              { them: "Disagreements go to WhatsApp",                      us: "Raise a dispute in-app — payer accepts, split updates automatically" },
            ].map((row, i) => (
        ...
        <FadeIn delay={80} className="flex flex-wrap items-center justify-center gap-3">
          {["AI expense parsing", "Chat import", "Voice input", "Live split preview", "Recurring templates", "Guest members", "QR code invites", "Import members", "Per-group insights", "Trip timeline", "Email & push alerts", "UPI pay links", "Debt flow graph", "CSV export", "Expense audit trail", "Installs on any device", "Inline comments", "In-app dispute resolution"].map((pill) => (
            <span key={pill} className="glass-sm rounded-full px-4 py-1.5 text-sm text-slate-600 dark:text-slate-300 border border-white/60 dark:border-slate-700/40">{pill}</span>
          ))}
        </FadeIn>
      </section>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <FadeIn direction="left" className="flex-1 w-full max-w-md">
            <div className="glass rounded-2xl p-5 shadow-xl shadow-cyan-500/10">
              ...
              <div className="rounded-2xl px-3 pt-3 pb-2" style={{ backgroundColor: "#2563EB12" }}>
                ...
                <div className="space-y-px">
                  <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-xs text-slate-700 dark:text-slate-200 truncate">Hotel check-in</p><p className="text-[10px] text-slate-400">Priya</p></div>
              ...
              <div className="flex justify-center py-0.5 my-0.5"><div className="w-px h-3 bg-gradient-to-b from-transparent via-slate-200 dark:via-slate-700/50 to-transparent" /></div>
              {/* Day 2 */}
              <div className="rounded-2xl px-3 pt-3 pb-2" style={{ backgroundColor: "#16A34A12" }}>
                ...
                <div className="space-y-px">
                  <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-xs text-slate-700 dark:text-slate-200 truncate">Water sports</p><p className="text-[10px] text-slate-400">Raj</p></div>
                    <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 tabular-nums shrink-0">₹8,000</span>
                  </div>
                  <p className="text-[10px] text-slate-300 dark:text-slate-600 italic px-2 pb-1">+2 more</p>
                </div>
              </div>
              <div className="flex justify-center py-0.5 my-0.5"><div className="w-px h-3 bg-gradient-to-b from-transparent via-slate-200 dark:via-slate-700/50 to-transparent" /></div>
              {/* Day 3 */}
              <div className="rounded-2xl px-3 pt-3 pb-2" style={{ backgroundColor: "#DB277712" }}>
                ...
                <div className="space-y-px">
                  <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-xs text-slate-700 dark:text-slate-200 truncate">Souvenirs</p><p className="text-[10px] text-slate-400">Anil</p></div>
          ...
          <FadeIn direction="right" className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 leading-relaxed max-w-md mx-auto lg:mx-0 mb-6">
              The timeline places every expense on the day it happened — no digging, no sorting. See the shape of your trip at a glance.
            </p>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <FadeIn direction="left" className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 leading-relaxed max-w-md mx-auto lg:mx-0 mb-6">
              Describe an expense the way you&apos;d text a friend — AI extracts the amount, payer, and split instantly.
            </p>
            <ul className="space-y-3 text-left max-w-md mx-auto lg:mx-0">
              {[
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <FadeIn direction="left" className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 leading-relaxed max-w-md mx-auto lg:mx-0 mb-6">
              Every time a group member logs an expense, everyone gets notified — by email and as an instant push alert on their phone.
            </p>
            <ul className="space-y-3 text-left max-w-md mx-auto lg:mx-0">
              {[
          ...
          <FadeIn direction="right" className="flex-1 w-full max-w-sm">
            <div className="glass rounded-2xl p-6 shadow-xl shadow-cyan-500/10">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-1.5">
                  {[0,1,2].map(i => <div key={i} className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600" />)}
                </div>
              </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <FadeIn direction="left" className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 leading-relaxed max-w-md mx-auto lg:mx-0 mb-6">
              Question an expense, request a split change, or dispute your share — right on the expense.
            </p>
            <ul className="space-y-3 text-left max-w-md mx-auto lg:mx-0">
              {[
          ...
          <FadeIn direction="right" className="flex-1 w-full max-w-sm">
            <div className="glass rounded-2xl p-5 shadow-xl shadow-cyan-500/10">
              ...
              <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-3.5 mb-4">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 mb-3 leading-relaxed">Raj: I only used the room for 2 nights — can my share be updated?</p>
                <div className="flex gap-2">
                  <button className="flex-1 text-xs font-semibold text-white py-1.5 rounded-lg" style={{ background: "linear-gradient(135deg, #06B6D4, #0D9488)" }}>Accept &amp; update split</button>
                  <button className="flex-1 text-xs font-medium text-slate-500 dark:text-slate-400 py-1.5 rounded-lg bg-white/60 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">Decline</button>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex gap-2">
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <FadeIn className="text-center mb-12">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
            Clear&apos;s algorithm collapses any tangle of shared expenses into the minimum number of transfers.
          </p>
        </FadeIn>
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_64px_1fr] gap-4 items-start max-w-3xl mx-auto">
          ...
          <FadeIn direction="right" className="glass rounded-2xl p-6 border border-teal-200/40 dark:border-teal-800/30">
            ...
            <div className="flex items-center gap-2 bg-white/60 dark:bg-slate-800/60 rounded-xl px-3 py-2.5 border border-slate-100 dark:border-slate-700/40">
              ...
              <div className="shrink-0 text-[10px] font-bold text-white px-2.5 py-1 rounded-lg" style={{ background: "linear-gradient(135deg, #06B6D4, #14B8A6)" }}>PAY →</div>
            </div>
          </FadeIn>
        </div>
      </section>

      {/* ── Debt Flow graph showcase ─────────────────────────────────────── */}
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <FadeIn direction="left"><SettleFlowDemo /></FadeIn>
          <FadeIn direction="right" className="text-center lg:text-left">
            ...
            <h2 className="text-3xl sm:text-4xl text-slate-800 dark:text-slate-100 mb-4 leading-snug" style={{ fontFamily: "var(--font-fraunces)" }}>
              See every balance
              <br />
              <span style={{ background: "linear-gradient(135deg, #0891B2 0%, #14B8A6 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                at a glance.
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 mb-8">The Debt Flow graph maps out who owes whom — animated money flows make it instant to understand, even in a group of ten.</p>
            <ul className="space-y-4 text-left max-w-sm mx-auto lg:mx-0">
              {[
                { emoji: "💫", label: "Animated money flows", desc: "Particles pulse from payer to recipient so the direction is always obvious." },
                { emoji: "👆", label: "Tap arc → jump to payment", desc: "Tap any arc and the exact payment card scrolls into view — one tap, done." },
                { emoji: "🖐", label: "Drag to untangle", desc: "Rearrange nodes freely when the graph gets crowded. It springs back neatly." },
              ].map((item) => (
                <li key={item.label} className="flex items-start gap-3">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <FadeIn direction="left" className="flex-1 w-full max-w-sm">
            <div className="glass rounded-2xl p-6 shadow-xl shadow-cyan-500/10">
              ...
              <div className="space-y-2">
                {[
                  { name: "Priya", amount: "₹8,000", pct: 96 }, { name: "You",   amount: "₹6,000", pct: 72 },
                  { name: "Raj",   amount: "₹4,000", pct: 48 }, { name: "Anil",  amount: "₹4,000", pct: 48 },
        ...
        <FadeIn>
          <div className="glass rounded-2xl px-8 py-7 flex flex-col sm:flex-row items-center justify-between gap-6 border border-violet-200/40 dark:border-violet-800/30">
            <div className="text-center sm:text-left">
              ...
              <p className="text-slate-700 dark:text-slate-200 font-medium mb-1">Need more room? Unlock unlimited groups, AI parsing, CSV export, and more.</p>
              <p className="text-sm text-slate-400 dark:text-slate-500">₹79/mo · or ₹699/yr (₹58/mo) · 30-day free trial · No credit card required.</p>
            </div>
            <Link href="/pricing" className="shrink-0 inline-flex items-center gap-2 bg-gradient-to-br from-indigo-500 to-violet-500 hover:from-indigo-600 hover:to-violet-600 text-white font-semibold text-sm py-2.5 px-6 rounded-xl shadow-md shadow-violet-500/20 transition-all hover:-translate-y-0.5 whitespace-nowrap">
        ...
        <FadeIn>
          <div className="relative rounded-3xl overflow-hidden px-8 py-16 text-center" style={{ background: "linear-gradient(135deg, #0E7490 0%, #0D9488 50%, #059669 100%)" }}>
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl sm:text-5xl text-white mb-4" style={{ fontFamily: "var(--font-fraunces)" }}>Ready to get clear?</h2>
              <p className="text-teal-100 text-lg mb-10 max-w-sm mx-auto">Create a trip or a nest in seconds. No credit card required.</p>
      ...
      <footer className="border-t border-white/40 dark:border-slate-700/40 bg-white/20 dark:bg-slate-900/20 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            <ClearLogo iconSize={28} wordmarkClassName="text-sm font-semibold text-slate-700 dark:text-slate-200" className="flex items-center gap-2" />
          ...
          <div className="flex items-center gap-5 text-xs text-slate-400 dark:text-slate-500">
            <Link href="/" className="hover:text-slate-600 dark:hover:text-slate-300 transition-colors">Overview</Link>
