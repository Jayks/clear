# ClearOff — Component & Design System Reference
...
No-auth shareable payment link. Reads `?to=userId&am=...`. Server-fetches payee name + UPI ID. `UpiPayButton` + return-from-UPI prompt on client. `app/pay/opengraph-image.tsx` for WhatsApp OG preview (ASCII-safe only — no `₹` in OG image). Chrome is `PublicPageShell` (see below) — no bespoke top nav/gradient background anymore (2026-07-02 unification).

### `PublicPageShell` (`components/shared/public-page-shell.tsx`)
Shared chrome for the three no-auth "someone is asking you to do something" pages: `/pay`, `/request/[token]`, `/stream/confirm/[token]`. Centered `ClearLogo` (unlinked — deliberately no easy way to navigate away mid-task) + card content (`children`) + optional soft acquisition footer (`footerText`/`footerCtaHref`/`footerCtaLabel`). Unifies chrome only — each page's payment/confirmation *mechanics* stay separate (`PayClient`'s return-from-UPI countdown, `RequestClient`'s self-report cash/bank/flexi flow, `ConfirmStreamClient`'s confirm/dispute) since those genuinely differ. `/join/[token]` deliberately does NOT use this shell — it's a real content preview (cover photo, member count, expense stats), not a status-message card, so a different layout is warranted there.
...
### Focus-visible — every `outline-none` needs a paired ring

Most form inputs already pair `outline-none` with `focus:ring-2 focus:ring-{accent}` (shows on any focus, including a mouse click — acceptable, not ideal). A 2026-07-02 audit found 7 elements — mostly hero "amount" inputs styled with a bare `border-b`/no border for visual weight (`PaymentSheet`, `StreamSettleSheet`, `StreamLogSheet`, `request-client.tsx`) plus a few chromeless search/comment inputs (`HomeControlBar`, `LocationInput`, `ThreadCommentInput`) — that had **zero** visible indicator on keyboard focus (`outline-none` with no ring at all). Fixed by adding `focus-visible:ring-2 focus-visible:ring-{accent}-400/50 rounded-{md|lg|xl}` (keyboard-only, doesn't add a ring on mouse click, matches the existing per-file/per-surface accent color). When adding any new bare/chromeless input (no visible border, or `outline-none` with nothing else), always pair it with a `focus-visible:ring-2` — don't rely on the browser default, since `outline-none` unconditionally suppresses it (Tailwind's `outline-none` sets `outline: 2px solid transparent`, not just `:focus`).

### Brand components — the "B-Converge" mark
- The mark = a **chamfered boxy C** (= Clear) — flat left side, 45° cut corners, two 45° lips cupping the node (`PATH_C`, `stroke-linejoin="miter"`) — with **two inflow strokes** (the L + r) converging into a node. Redesigned 2026-06-22 (`190ad3b`) from a flat split-node ("e"/"a" halves) to a **darker, 3D-bevelled** glyph with a **checkmark cut into the node** (`CHECK_PATH`) reading as "cleared off". The bevel is faked (Satori can't do lighting filters/blur): `buildBevelLayers()` stamps `layers+1` diagonally-offset, darkening duplicate copies of the C/inflow-strokes/node behind the front layer — pure shape stacking, renders identically in `<svg>` and inside Satori. Every letter of "Clear" is still hidden in the skeleton (C + L + r + node).
- **Geometry lives once, in `lib/brand-glyph.ts`** (pure data/math, no JSX/React — safe to import from browser, Node, and Edge/Satori runtimes alike): `PATH_C`, `INFLOW_1/2`, `NODE_CX/CY/R`, `HALO_R`, `CHECK_PATH`, `HIGHLIGHT`, `GLYPH_GRADIENT`, `GLYPH_DARK`, `buildBevelLayers()`. Every surface that renders the mark imports from this one module instead of inlining duplicate path data: `components/shared/clear-logo.tsx` (canonical `ClearIcon`/`ClearLogo`), `app/icon.tsx` (favicon), `app/api/pwa-icon/route.ts` (PWA/apple-touch), `app/api/settle-card/route.ts`, `app/pay/opengraph-image.tsx`, `app/summary/[token]/opengraph-image.tsx`, `app/request/[token]/opengraph-image.tsx`.
- `ClearLogo` (`components/shared/clear-logo.tsx`) — gradient **glass** icon box (specular bloom + rim overlays) + optional **two-tone wordmark** (`BRAND.namePrefix` plain + `BRAND.nameAccent` in `text-cyan-600 dark:text-cyan-400`). Props: `iconSize`, `showWordmark`, `wordmarkClassName`, `className`.
...
- Icon gradient — now a **darker** cyan→near-black glass, matching the "3D-bevelled" redesign: `GLYPH_GRADIENT = "linear-gradient(140deg, #129DB8 0%, #07788C 42%, #08596A 78%, #062F38 100%)"` (`lib/brand-glyph.ts`) — use inline style (not Tailwind) to keep exact stops. `GLYPH_DARK` (`#062F38`, the gradient's darkest stop) doubles as the bevel shadow colour and the checkmark's "cut" colour. The marketing CTA gradient (`from-[#129DB8] to-[#07788C]`, above) deliberately echoes this icon gradient's first two stops.
- PWA / apple-touch icons: `app/api/pwa-icon/route.ts` (192+512, edge). Favicon: `app/icon.tsx` (32px). Both layer the glass material (gradient base + specular + vignette + rim) since Satori can't do SVG blur/clip. OG images (`app/pay`, `app/summary`, `app/request/[token]`) + `app/api/settle-card` inline the same glyph via `lib/brand-glyph.ts`. The bevel/checkmark detail intentionally simplifies at favicon size.

### Navigation
- **Desktop**: collapsible left rail (`app/(app)/app-sidebar.tsx`, `AppSidebar`) — replaced the old horizontal `AppNav` top bar entirely. `ClearLogo` (collapses to icon-only) + collapse toggle at top. Nav body: **Trips** (`/groups?type=trips`, cyan) · **Nests** (`/groups?type=nests`, emerald) · **Circles** (`/groups?type=circles`, violet) as first-class section shortcuts at the top (`GROUP_TYPE_LINKS`), then a separator, then **Streams** · **Insights** (`FEATURE_NAV_LINKS`). Logo always links to `/groups` (all-types overview) — no explicit "Home" nav item. Bottom cluster (2026-07-03: `Bell → ThemeToggle → Avatar`) pinned at the bottom — see Notifications bell below. Collapsed state persisted in `localStorage` key `clear_sidebar_collapsed` (read in a `useEffect` after mount — server always renders expanded, same SSR-can't-know-localStorage pattern as other dismissable UI in this app). Avatar dropdown opens `side="right" align="end"` (not the default `bottom`) since the trigger sits near the bottom-left corner of the viewport. Carries the same Streams unread/dispute badge dot `MobileNav` shows (`clear_stream_has_badge` — amber=disputed, emerald=new) since it's now a permanent fixture rather than an easy-to-miss top bar.
...
- **Mobile top nav**: icon-only `AppNav` — **mobile-only now** (`md:hidden` unconditionally; desktop uses `AppSidebar` instead, so `AppNav` no longer renders its own copy of the Home/Streams/Insights links at all). Still hidden entirely (`hidden`, both viewports) on group detail pages (`isInsideGroup`) AND stream pages (`isInsideStream`) — those have their own custom sticky headers on mobile, and `AppSidebar` already covers desktop nav for them. Right side (2026-07-03): `ThemeToggle → Bell → Avatar` — see Notifications bell below.

### Notifications bell (2026-07-03)

`NOTIFiCATIONS_INBOX_PLAN.md` §2.4 Phase 2, built against the `notifications` table (see `lib/db/CLAUDE.md`). Platform-split same as `GroupSwitcherDropdown`/`GroupSwitcherSheet`:
- **`components/notifications/notification-bell-desktop.tsx`** — lives in `AppSidebar`'s bottom cluster. Anchored `DropdownMenu` (base-ui `Menu`, `side="top" align="start"` — opens above the bottom cluster rather than sideways like the avatar menu next to it; the notification list reads better stacked above the row it came from since it's taller than the avatar menu's content). **List + badge re-fetch on every open** (Round 16 fix #5, 2026-07-06) via `getNotificationFeedAction()` (one round-trip, rows + unread count together) — not just the first open, so a long-lived tab/PWA doesn't show morning data all day. The previously-cached list stays rendered while a refresh is in flight (only the very first-ever open shows the spinner). A failed fetch sets a `loadFailed` flag and shows a "close and reopen to retry" message instead of an eternal spinner (fix #16).
- **`components/notifications/notification-bell-mobile.tsx`** — lives in `AppNav`'s top bar. Uses the shared `Sheet` primitive (`components/shared/sheet.tsx`) rather than hand-rolling like the older `GroupSwitcherSheet` — focus trap, Escape, drag-to-dismiss, iOS scroll-lock all come free. Same refresh-on-every-open + `loadFailed` retry-copy behaviour as the desktop bell (Round 16 fixes #5/#16).
- Both own their own `unread` count `useState`, seeded from an `initialUnread` prop (server-fetched once in `app/(app)/layout.tsx` via `getUnreadNotificationCount`, passed down to both `AppSidebar`/`AppNav`), and optimistically decrement/zero it on row-click / "Mark all read" without waiting for a server round-trip.
- **Cross-surface read-state sync (2026-07-03 bugfix)** — `AppSidebar` and `AppNav` are both always mounted (CSS `md:hidden`-toggled, not conditionally rendered), and the full `/notifications` page lives inside that same persistent layout, so all three notification surfaces are alive simultaneously with fully independent local state. Marking read on one surface (e.g. "Mark all read" on the full page) has no way to reach the others' badge/cached list without an explicit bridge — same class of problem the Stream badge already solved via `clear_stream_has_badge` + a custom event. Fix: `lib/notifications/notification-sync.ts` (`broadcastNotificationsRead({ scope: "all" | "one" })`, a plain `window` `CustomEvent` — no localStorage needed, a fresh load always re-derives truth from the DB) + `hooks/use-notification-read-sync.ts` (`useNotificationReadSync(setNotifications, setUnread?)`, one listener per surface that applies every broadcast — including its own — to local state). `NotificationRow` broadcasts `{ scope: "one", id }` itself right where it fires `markNotificationReadAction`, so no per-surface wiring is needed for the single-row case; each bell's `onOpen` callback now only handles its own surface-specific concern (mobile: closing the sheet).
- **`components/notifications/notification-row.tsx`** — the one shared row renderer (icon badge via `lib/notifications/notification-meta.ts`'s `NotificationType → {icon, iconClass, bgClass}` map, title, body, relative time) used identically by both bell surfaces AND the full `/notifications` page (`app/(app)/notifications/`) — so all three read the same visual language. Tapping a row fires `markNotificationReadAction` fire-and-forget, then follows the notification's own `url` via a plain `<Link>` (no `preventDefault`/custom nav needed, unlike `GroupSwitcherDropdown`'s section-preserving redirect).
- **`components/notifications/notification-list.tsx`** (2026-07-03) — the shared read/unread grouping, used by all three surfaces alongside `NotificationRow`. Partitions the already-fetched list (no extra query) into a **"New"** group (unread) and an **"Earlier"** group (read), preserving each group's existing reverse-chronological order; "Earlier" only gets its own header when both groups are non-empty (an all-unread or all-read list renders as one plain group, same as the old flat list). **"Mark all read" lives inside the New header**, not a separate page/panel-level button — it visibly only ever acts on the unread group, matching what `markAllNotificationsReadAction` actually does server-side. Optional `renderItem` prop lets the desktop bell keep wrapping each row in a `DropdownMenuItem` (arrow-key nav + auto-close semantics from the base-ui `Menu`) without the mobile sheet / full page needing that wrapping at all.
- Icon badge colours are grouped by domain to match the app's existing colour-identity system rather than inventing a new one: violet=Circles, blue=Streams, cyan=Expenses/comments, amber=disputes, emerald=Settle.
- **App nav bars are transparent**: `AppNav`, `MobileNav`, `GroupMobileNav` all use `backdrop-blur-sm` (no background, no border); `AppSidebar` is the one exception (`bg-white/80 dark:bg-slate-950/80` + `border-r` — a sidebar needs a visible edge against content, a top bar didn't). **Marketing/public pages** (`/`, `/pricing`, `/changelog`, `/admin`) still use `.glass-nav`. Do NOT use `.glass-nav` on in-app navbars.
...
- **Root layout shell** (`app/(app)/layout.tsx`): `flex flex-col md:flex-row` — mobile keeps the old vertical stack (`AppNav` top, `MobileNav` bottom); desktop goes horizontal (`AppSidebar` rail on the left, a `flex-1 min-w-0` content column on the right holding `TrialBanner` + `<main>`). `min-w-0` on the content column matters — without it, wide content inside a flex row can force the whole row to overflow instead of letting the column shrink/scroll.
- **Sticky offsets that assumed `AppNav`'s `h-14` sits above them on desktop now need a `md:` override** (or, if the element is already desktop-only, just `top-0`) since `AppSidebar` is a side rail with zero vertical footprint: `GroupDesktopNav`'s wrapper (`top-0`, desktop-only so no `md:` split needed), `SectionPillNav` (`top-14 md:top-0` — still needs `top-14` on mobile where `AppNav` is unchanged), Settings page's local `<aside>` (`top-20` → `top-6`, was AppNav's 56px + 24px breathing room, now just the breathing room). Check before adding any NEW desktop-only sticky element under the content column — `top-0` is almost always right, not the AppNav-height value that was true before this redesign.
- **Within group routes on mobile**: `AppNav` hides (`hidden` — see above); `GroupMobileNav` renders inside `<main>` as `sticky top-0 z-40 -mx-6 -mt-6` (negative margins break out of padding; sticky scrolls past TrialBanner). Header = **‹ Back · group-name ▾ (switcher) · ⋯**. **Back is HIERARCHICAL** (`router.push(backHref)`, never `router.back()`): section → overview, deep pages (add/edit/thread/templates) → their list, overview → Home — matches the label shown, so the bottom-nav hopping never feels like "going in circles". The centre is the **group-name ▾ switcher** on the overview + four section pages (the highlighted bottom-nav tab tells you the section, so the header doesn't repeat it); deep task pages show the **task title + section icon** instead (no switcher). `resolveNav(pathname, groupId)` returns `{ switcher, section, pageTitle, icon, backHref, backLabel }`.
...
- **Back navigation — deterministic push, never `router.back()`, on the desktop "Back to X" control or on history-shape assumptions.** `BackButton` (`components/shared/back-button.tsx`) pushes its known, deterministic `href` (`router.push(href)`) instead of popping browser history — matches the mobile hierarchical model (`resolveNav` above) instead of trusting whatever the actual history stack happens to contain. **Form save-handlers are the other half of this**: a page reached by *pushing* forward from a list/overview (Edit group, Edit/Add expense, Edit/Add recurring template, Add circle wallet expense) calls `router.back()` on successful save — NOT `router.push`/`router.replace` to the same URL the list page already has. Pushing/replacing to that URL writes a **second, adjacent history entry with an identical URL** sitting next to the original — the browser back button's first press then pops between the two same-URL entries (visibly "does nothing"), and only the second press actually leaves. `router.back()` instead pops the form's own pushed entry cleanly, landing on whichever parent it actually came from (works correctly even for multi-entry-point forms like Add Expense, reachable from either the group's Expenses list or Home's `?from=groups`). The one exception: forms that create something brand-new (`create-trip-form.tsx`, `repeat-trip-prompt.tsx`) correctly use `router.replace()` to the newly-created group's URL — that target was never the immediately-prior entry, so no duplicate risk.

- **In-group navigation (the contextual nav model)** — three jobs, one home each:
  - **Navigate within the group**: **`GroupBottomNav`** (mobile, `components/shared/group-bottom-nav.tsx`) — while inside a group the global `MobileNav` hides (`mobile-nav.tsx` returns null when `pathname` is `/groups/[id]/…`, not `/groups` or `/groups/new`) and this contextual bar takes its place: Overview · Expenses · Settle · Members · Insights, context-coloured sliding pill (`layoutId="group-nav-pill"`, `theme.navPill`). **`GroupDesktopNav`** (`group-desktop-nav.tsx`) is the desktop counterpart — a sticky tab strip (rendered by the group layout in a `hidden md:block sticky top-0 z-30 -mt-8` wrapper — `top-0` because `AppSidebar` has no vertical footprint above it, unlike the old `AppNav`), `rounded-2xl` + full `border` + `shadow-sm` (a floating card, not a full-bleed bar — its earlier full-bleed background didn't line up with the rest of the page's margins, which read as a layout bug). Group-name switcher gets `pl-4` breathing room from the rounded left edge; tabs sit in a `flex-1 justify-center` block (centred in the remaining width, not packed against the divider) and get a context-gradient underline on the active one. A **`⋯` button** at the right end opens the same `GroupActionHub` as `GroupMobileNav`'s ⋯ (`showJumpTo={false}`) — added because the only previous desktop entry point to Edit/Archive/Share was `GroupHeroHub` on the Overview page specifically, so a desktop user on Expenses/Settle/Members/Insights had no way to reach group management without navigating back to Overview first. Both nav bars use **optimistic active** (set the tapped section in local state so the indicator moves on click, not after the route commits — fixes jumpiness) and the **shared tab list `getGroupTabs(groupId, groupType)`** (`lib/nav/group-tabs.ts`, circle-aware: circles drop Settle/Insights). **`GroupBottomNav` portals to `document.body`** — required because the group layout sits inside `PageTransition` (keyed on pathname), whose per-nav opacity/translate replay was flickering the fixed bar; portaling moves it out of that subtree. The portal is guarded by a `mounted` state (`useState(false)` + `useEffect(() => setMounted(true), [])`, return `null` until mounted) — **never use `typeof document !== "undefined"`** for this: that's `false` on the server but `true` on the client during hydration, which causes a React hydration mismatch and forces a full client re-render.
  - **Switch between groups**: **`GroupSwitcherSheet`** (`group-switcher-sheet.tsx`) — **mobile only now**, opened by the group-name ▾ in `GroupMobileNav`. **`GroupSwitcherDropdown`** (`group-switcher-dropdown.tsx`) is the desktop counterpart, opened by `GroupDesktopNav`'s group-name ▾ — a proper anchored `DropdownMenu` (base-ui `Menu`) instead of reusing the mobile bottom-sheet pattern, which looked like a misplaced mobile control sliding up on a wide viewport. **Do not use `DropdownMenuLabel` without wrapping it in `DropdownMenuGroup`** — base-ui's `Menu.GroupLabel` throws `"MenuGroupRootContext is missing"` if used standalone; use a plain styled `<p>` for non-interactive header text instead (no group semantics needed for a single label). **Shared logic extracted 2026-07-02**: both components now share `hooks/use-switcher-groups.ts` (the lazy `getSwitcherGroups()` fetch — `ensureLoaded()` is idempotent, safe from either a mount effect or an `onOpenChange` callback) and `lib/nav/group-switcher.ts` (pure `typeLabel`/`resolveSwitchSection`/`targetHref`/`stashSwitcherSection`, unit-tested in `group-switcher.test.ts`) instead of each hand-rolling its own copy. **Section-preserving**: switching from Settle lands on the target's Settle, falling back to overview for Circles (no Settle/Insights). The sheet shows the existing `NavProgress` bar + an in-row "Switching…" state; auto-closes when `groupId` changes; **deliberately NOT `useSheetDismiss`** (its fake-history entry would pollute the back stack on the selection navigation) — plain Escape handler + `useFocusTrap` instead. **Back-navigation correctness (sessionStorage handshake, both components)**: when switching from a section page, store `{ groupId, section }` under `sessionStorage.key "clearSwitcherSection"` then call `router.replace("/groups/B")`. `GroupMobileNav`'s/`GroupDesktopNav`'s own `useEffect([pathname, groupId])` reads the key on landing, validates `groupId` matches and `pathname === "/groups/B"`, removes the key, and calls `router.push("/groups/B/section")`. Resulting history: `[..., /groups/A/settle, /groups/B, /groups/B/settle]` → back → `/groups/B` overview ✓. `setTimeout(0)` was tried first but is unreliable — App Router can cancel the `replace` when the `push` fires before it commits; sessionStorage decouples the two navigations.
  - **Act on the group**: the **`⋯` `GroupActionHub`** — Zone 2 "Jump to" is now gated by `showJumpTo` (default true). Kept on the **home-page card ⋯** (a deep-link shortcut; no bottom nav there); passed `showJumpTo={false}` from the inside-group entry points (`GroupMobileNav`, `GroupDesktopNav`, `GroupHeroHub`) where the bottom nav / tab strip / overview cards already cover it.
...
**Mobile-vs-desktop is decided by real touch capability (`window.matchMedia("(pointer: coarse)")`), not viewport width** — a Windows touchscreen laptop with a narrow browser window still gets the desktop hover-reveal variant if the input is a mouse/trackpad, and a wide DevTools-emulated mobile viewport gets the swipe variant if touch emulation is on. The `isTouchDevice` check **must be a live `matchMedia` change-listener, not a mount-once read** (`mq.addEventListener("change", ...)`, not just `setIsTouchDevice(mq.matches)` in an empty-deps effect) — a one-shot read freezes at whatever was true when the card first mounted, so toggling Chrome DevTools' device toolbar *after* the page already loaded silently does nothing without a full reload (looked like "swipe-to-delete just disappeared" when actually it never re-evaluated).

**`ExpenseDetailSheet`'s Edit pencil race** — the Edit `<Link>` in the sheet header must NOT call `onClose()` (closing the sheet) and rely on the `<Link>`'s own forward navigation on the same click. `ExpenseDetailSheet` uses `useSheetDismiss`, which pops its fake history entry **asynchronously** (`history.go(-1)`, resolves on a later tick) when `isOpen` flips to `false`. If that async pop resolves *after* the Link's `router.push` already landed, it silently undoes the navigation — the button visibly "does nothing" (it briefly navigates, then gets yanked back). Fix: intercept the click (`e.preventDefault()`) and call `router.replace(href)` directly, skipping `onClose()` entirely — the route change unmounts the sheet naturally, and the unmount-time cleanup sees the *new* route's history state (not the fake `bottomSheet` one) so it correctly skips the `go(-1)`.
...
### ExpenseFilters — mobile filter panel

`components/expense/expense-filters.tsx` — the search/sort/category/payer/date row collapses very differently on mobile vs desktop:
- **Desktop**: everything inline and permanently visible (search + sort row, then category pills, payer pills/dropdown, date range — all `hidden md:block` below the search row). Plenty of room, no need to hide anything.
- **Mobile**: only a search bar + a ⚙️ Filter button (`SlidersHorizontal` icon, dot badge via `hasActiveNonSearchFilters` when category/payer/date is set) show by default. Tapping the button toggles `filtersExpanded` (state, not a sheet) — an **inline accordion panel** (`AnimatePresence` + `motion.div` animating `height: 0 → "auto"`) expands directly in the page flow with Sort/Category/Paid by/Date, pushing the expense list down. **Deliberately not a bottom-sheet/drawer** — an overlay would require a dismiss step before seeing the filtered results, which defeats live filtering; the inline panel keeps the list reachable (scroll past it) the whole time it's open. Sort and Date range use a single-line `label beside control` layout (not label-above-control) to stay compact.
- The map view mode previously had its own separate mobile filter drawer (floating chip + bottom sheet) — this was generalised into the single `filtersExpanded` panel above for every view mode; the map-specific drawer no longer exists.
...
Sits `sticky top-14 md:top-0 z-40 -mx-6 px-6 md:-mx-8 md:px-8 backdrop-blur-sm` — `top-14` on mobile (where `AppNav` is unchanged, still occupies 56px above it), `top-0` on desktop (`AppSidebar` is a side rail with no vertical footprint). Pill size: `px-4 py-2 text-sm` (was `px-3 py-1.5 text-xs` — bumped for better tap targets). Active pill gets accent colour: cyan=Trips, emerald=Nests, violet=Circle, **amber=Archived** (slate looked disabled; amber is unambiguous). `SectionColor` type: `"cyan" | "emerald" | "violet" | "amber" | "slate"`. Sections need `scroll-mt-28` to clear both the AppNav and this sticky bar.
...
### SettleFlowDemo
`components/marketing/settle-flow-demo.tsx` — static version of `DebtFlowGraph` (same arc math, SMIL particles, `AVATAR_COLORS`; no drag/selection; hardcoded data). Used in `AboutLanding` (desktop `/`) and inside `CarouselLanding` slide 3 (`PhoneFrame` + dark `AppBar`). Same SVG+Framer Motion two-wrapper fix applies.
...
**Second gotcha — a Link inside the sheet that also calls `onClose()` races the async pop.** `useSheetDismiss`'s cleanup does `history.go(-1)` (async — resolves on a *later* tick), so if a click handler both closes the sheet (`onClose()`) AND triggers a `<Link>`'s forward navigation on the same click, the delayed pop can resolve after the navigation already landed and silently undo it (button looks like it "does nothing"). See `ExpenseDetailSheet`'s Edit pencil fix under `SwipeableExpenseCard` above — skip `onClose()` for any in-sheet navigation link, intercept the click, and `router.replace()`/`router.push()` directly instead.
