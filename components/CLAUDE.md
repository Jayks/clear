...
- **`components/notifications/notification-bell-desktop.tsx`** — lives in `AppSidebar`'s bottom cluster. Anchored `DropdownMenu` (base-ui `Menu`, `side="top" align="start"` — opens above the bottom cluster rather than sideways like the avatar menu next to it; the notification list reads better stacked above the row it came from since it's taller than the avatar menu's content). Lazy-loads the last 10 notifications on first open via `getNotificationsAction`.
- **`components/notifications/notification-bell-mobile.tsx`** — lives in `AppNav`'s top bar. Uses the shared `Sheet` primitive (`components/shared/sheet.tsx`) rather than hand-rolling like the older `GroupSwitcherSheet` — focus trap, Escape, drag-to-dismiss, iOS scroll-lock all come free.
