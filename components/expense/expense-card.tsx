...
export function ExpenseCard({ expense, members, currentUserId, currentMemberId, isAdmin, onDelete, onDeleteFail, interactionCount, hideActions = false, hoverRevealActions = false, compact = false }: ExpenseCardProps) {
  ...
  // Category accent — drives the left stripe colour
  const catMeta = getCategory(expense.category);
  const accentGradient = catMeta?.gradient ?? "from-slate-400 to-slate-500";

  return (
    // overflow-hidden + flex lets the stripe stretch to full card height
    <div className="glass rounded-xl overflow-hidden flex">
      {/* 3px category colour stripe */}
      <div className={`w-[3px] shrink-0 bg-gradient-to-b ${accentGradient}`} aria-hidden="true" />
      ...
      <div className="flex-1 px-4 py-3 flex flex-col gap-1 min-w-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {expense.category === "other" && expense.customCategory && (
              <span className="inline-block mt-0.5 px-1.5 py-0.5 text-[10px] rounded-md bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 font-medium leading-none">
          ...
          {/* Amount — emerald when current user paid, neutral otherwise. text-xl for visual weight. */}
          <p
            className={`text-xl font-semibold tabular-nums shrink-0 ${isPayer ? "text-emerald-600 dark:text-emerald-400" : "text-slate-800 dark:text-slate-100"}`}
        ...
        {!compact && (creatorName || canEdit) && (
          <div className="flex items-center justify-between gap-2" onClick={(e) => e.stopPropagation()}>
            {/* Left: who added (and edited) */}
            <p className="text-xs text-slate-400/70 dark:text-slate-500/70 truncate min-w-0">
              {creatorName && (
                <>
