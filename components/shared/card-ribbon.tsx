 ...
 * CircleCard so "SAMPLE" / "ARCHIVED" look identical everywhere.
 *
 * Render inside the card's inner `overflow-hidden` wrapper (so it clips to the
 * rounded corner and spans image + badges).
 */
export function CardRibbon({ variant }: { variant: "sample" | "archived" }) {
  ...
  return (
    <div
      className={`absolute bottom-[22px] right-[-30px] w-[130px] rotate-[-45deg]
                  ...
                  ${isSample ? "bg-amber-500/25" : "bg-slate-500/30"}`}
    >
      {isSample ? "SAMPLE" : "ARCHIVED"}
    </div>
  );
}
