...
**Round 16 (2026-07-06) — same pattern applied to the template-double-log guard**: `logFromTemplate`, `autoLogDueTemplates`, and `batchLogTemplates` (`app/actions/expenses.ts`) each wrapped a SELECT-then-INSERT "already logged this month?" guard in `db.transaction()` without a row lock — the identical race shape (two concurrent transactions both see "not logged" before either commits). Fixed by locking the *template row itself* (`await tx.select({id: expenses.id}).from(expenses).where(eq(expenses.id, templateId)).for("update")`) before the `alreadyLogged` check in all three functions. Also backed by a defense-in-depth `unique index` (`drizzle/template-log-dedup.sql`, applied manually) on `(source_template_id, expense_date) WHERE source_template_id IS NOT NULL AND is_template = false` — belt-and-suspenders in case a future code path re-introduces the unlocked guard.
